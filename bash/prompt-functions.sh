...
function __git_color_pathX()
{
  ...
  printf -v white '\[\e[1;36m\]'
  out="$PWD"
  gittop=$(git rev-parse --git-dir 2> /dev/null)
  while [ "$?" == "0" ]; do
    gittop=$(realpath -s "${gittop}"/..)
    if [ -d "${gittop}/.git/rebase-merge" -o \
         -d "${gittop}/.git/rebase-apply" -o \
         -f "${gittop}/.git/MERGE_HEAD" -o \
         -f "${gittop}/.git/CHERRY_PICK_HEAD" -o \
    ...
    gittop=$(git -C $gittop/.. rev-parse --git-dir 2> /dev/null)
  done
  reporoot=$(repo --show-toplevel 2> /dev/null)
  tail=${out#${reporoot}}
  if [ "$?" == 0 ]; then
